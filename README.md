...
机器人 play 测试障碍组件：高台、台阶、镂空台阶、斜坡、独立小方柱梅花桩、三角障碍、平放汽车轮胎圈、绕杆、沙坑、高墙。三角障碍默认按左、右、左、右交错，并带有横向错位参数 `stagger`，避免斜面完全正对。

默认尺寸约定：轮胎圈外径 `0.74 m`、小方柱边长 `0.45 m`；这些尺寸都可以在 PyQt 编辑器右侧障碍参数中修改。
...
编辑器使用蓝白色界面，所有操作都以障碍对象为中心：左侧选择新增障碍类型和对象列表，中间是场地俯视预览，右侧编辑当前障碍的参数。点击空白处按当前类型新增，点击障碍即可选中；按住 Ctrl 点击障碍进入单对象编辑，可直接拖动障碍改变位置，使用右侧旋转按钮改变朝向，最后点击“保存障碍”或“删除”。不再提供独立的画布操作模式。
